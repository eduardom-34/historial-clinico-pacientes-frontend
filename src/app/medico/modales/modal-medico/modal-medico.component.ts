...
export class ModalMedicoComponent implements OnInit {
  ...
  crearModificarMedico() {
    ...
    if( this.datosMedico == null ) //nuevo medico
    {
      this._medicoServicio.crear(medico).subscribe({
        next: (data) => {
          if(data.isExitoso){
            this._compartidoServicio.mostrarAlerta('Medico ha sido gravado con Exito!', 'Completo');
      ...
      this._medicoServicio.editar(medico).subscribe({
        next: (data) => {
          if(data.isExitoso){
            this._compartidoServicio.mostrarAlerta('Medico ha sido actualizaco con Exito!', 'Completo');
            this.modal.close("true");
          } else {
