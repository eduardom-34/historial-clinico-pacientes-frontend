...
export class ModalUsuarioComponent {
  ...
  constructor(private modal: MatDialogRef<ModalUsuarioComponent>,
    @Inject(MAT_DIALOG_DATA) public datoUsuario: Registro,
  ...
  this._usuarioServicio.listadoRoles().subscribe({
    next: (data) => {
      if( data.isExitoso ) {
        this.listaRoles = data.resultado;
      }
    },
    error: (e) => {}
  });
 ...
 registrarUsuario() {
  ...
  this._usuarioServicio.registrar(usuario).subscribe({
    next: (data) => {
      this._compartidoServicio.mostrarAlerta('Usuario se ha registrado con Exito!', 'Completo');
